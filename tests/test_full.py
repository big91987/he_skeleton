...
class FullWorkflowTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.work / "validation.md").write_text(
            "Actual validation scope and evidence. " * 10
        )
        self.cfg = {
            "max_attempts": 2,
            "agent_timeout": 60,
            ...
            "entries": ["AGENTS.md"],
            "stages": {
                s: {"skills": [], "artifact": "validation.md"}
                for s in runner.STAGES[:3]
            },
            "checks": [
                {
                    "name": "real assertion",
        ...
        self.cp = self.root / "context.json"
    ...
    def test_python_quality_blocks_development_even_when_functional_check_passes(self):
        (self.work / "fixed").write_text("ok")
        (self.work / "app.py").write_text("x=1;print(x)\n")
        context_path = self.root / "context.json"
        write_json(context_path, self.context)
        result = stop_hook.evaluate(
            context_path,
            {
                "last_assistant_message": json.dumps(
                    {
        ...
        self.assertIn("Python formatting and lint", result["reason"])
    ...
    def test_failing_real_check_then_same_gate_pass(self):
        ...
        self.assertEqual(gate["snapshot"], digest(self.work))
    ...
    def test_no_applicable_implementation_check_blocks(self):
        self.context["config"]["checks"][0]["stages"] = ["design"]
        write_json(self.cp, self.context)
        stop_hook.evaluate(self.cp, self.ready)
        self.assertEqual(
            json.loads((self.evidence / "gate.json").read_text())["status"], "blocked"
    ...
    def test_clarification_does_not_run_checks(self):
        with patch.object(
            stop_hook, "run_process", side_effect=AssertionError("must not run")
        ):
            stop_hook.evaluate(
                self.cp,
                {
                    "last_assistant_message": json.dumps(
                        {"status": "needs_input", "question": "Which user role?"}
                    )
                },
            )
        self.assertEqual(
            json.loads((self.evidence / "gate.json").read_text())["status"],
            "needs_input",
        )
    ...
    def test_wall_clock_jump_does_not_expire_active_gate(self):
        ...
        with patch.object(stop_hook.time, "time", return_value=10**12):
            stop_hook.evaluate(self.cp, self.ready)
        self.assertEqual(
            json.loads((self.evidence / "gate.json").read_text())["status"], "passed"
        )
    ...
    def test_stale_reply_and_base_drift_rejected(self):
        ...
        with self.assertRaises(ValueError):
            runner.begin(state, "old yes", "sha", "machine", "1")
        with self.assertRaises(ValueError):
            runner.begin(state, "new yes", "changed", "machine", "1")
            ...
            if path.startswith("git/matching-refs"):
                return (
                    ...
                    if not remote["ref"]
                        ...
                        {
                            "ref": "refs/heads/codex/full-task-1",
                            "object": {"sha": remote["ref"]},
                        }
                    ]
                )
            if path == "git/trees":
