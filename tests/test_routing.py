...
class RoutingTests(unittest.TestCase):
                ...
                "checks": [
                    {
                        ...
                        "argv": [
                            sys.executable,
                            "-c",
                            'from pathlib import Path; assert Path("fixed").exists()',
                        ],
                    }
                ],
            },
    ...
    def decision(self, actions):
        return {
            "status": "ready",
            "summary": "Assessed actual materials",
            "question": "",
            "decisions": [
                {
                    "stage": stage,
                    "action": action,
                    "reason": "Based on current task and materials",
                    "evidence": (["code.py"] if stage == "development" else ["prd.md"])
                    if action == "reuse"
                    else [],
                }
    ...
    def apply(self, result):
        ...
        with patch.object(runner, "classify", return_value=result):
            runner.assess_entry(self.root, self.root, self.state)
    ...
    def test_missing_evidence_cannot_skip_stage(self):
        ...
        result["decisions"][0]["evidence"] = ["missing.md"]
    ...
    def test_current_issue_reference_is_canonical_through_approval(self):
        ...
        self.assertIn(
            "issue", runner.material_hashes(self.root, self.state, "requirements")
        )
    ...
    def test_escape_and_illegal_stage_are_rejected(self):
        result = self.decision(["reuse", "run", "run"])
        result["decisions"][0]["evidence"] = ["../outside"]
        with self.assertRaises(ValueError):
            router.validate(result, self.work, self.state)
        result = self.decision(["run"] * 3)
        result["decisions"][-1]["stage"] = "delivery"
        with self.assertRaises(ValueError):
            router.validate(result, self.work, self.state)
    ...
    def test_requirement_not_applicable_is_rejected(self):
        with self.assertRaises(ValueError):
            router.validate(
                self.decision(["not_applicable"] * 3), self.work, self.state
            )
        ...
        self.apply(
            {
                "status": "needs_input",
                "summary": "Need scope",
                "question": "Which interface is in scope?",
                "decisions": [],
            }
        )
        self.assertEqual(self.state["stage"], "entry")
    ...
    def test_existing_code_runs_real_check_without_builder(self):
        ...
        with patch.object(
            runner, "run_agent", side_effect=AssertionError("must not code")
        ):
            runner.verify_stage(self.root, self.root, self.state)
        self.assertEqual(self.state["stage"], "review")
        self.assertEqual(
            self.state["completed"]["verification"]["checks"][0]["code"], 0
        )
    ...
    def test_failure_returns_to_builder_then_rechecks(self):
        ...
        def repair(*args):
            (self.work / "fixed").write_text("repaired")
    ...
    def test_existing_code_with_bad_format_returns_to_builder(self):
        (self.work / "fixed").write_text("functional check passes")
        (self.work / "code.py").write_text("x=1;print(x)\n")
        self.state["completed"] = {"development": {"mode": "reuse"}}
        ...
        self.assertTrue(
            all(
                check["code"] == 0
                for check in self.state["completed"]["verification"]["checks"]
            )
        )

    def test_repeat_failures_stop_at_bound(self):
        with patch.object(runner, "run_agent") as builder:
            runner.verify_stage(self.root, self.root, self.state)
