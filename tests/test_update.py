...
class UpdateProject(unittest.TestCase):
    def test_pinned_sync_preserves_business_and_rejects_drift(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            (product / "app").mkdir()
            (product / "app/index.html").write_text("business")
            changes = prepare(source, product, "canonical-remote-sha")
            self.assertEqual(
                set(changes), set(MANAGED) | set(TEMPLATES) | {"harness-upstream.json"}
            )
            ...
            self.assertEqual(
                json.loads((product / "harness-upstream.json").read_text())["revision"],
                "canonical-remote-sha",
            )
