...
from full_harness.runner import STAGES, begin, new_state, review_stage, run_agent

source = SOURCE
if "--run-live" not in sys.argv:
    raise SystemExit(
        "Opt-in required: python3 tests/integration_full.py --run-live; uses authenticated Codex and model quota"
    )
root = Path(tempfile.mkdtemp(prefix="full-live-", dir=tempfile.gettempdir()))
print("probe_root", root, flush=True)
work = root / "conversation"
work.mkdir()
...
for i, prompt in enumerate(
    ...
    result, current = invoke(
        source, work, session, prompt, root / ("conversation-" + str(i)), session_id=sid
    )
    if sid:
        assert current == sid
    sid = current
    print("conversation", i, current, result, flush=True)
assert all(x in result["summary"] for x in ["BlueShelf", "蓝", "紧凑"])
project = root / "project"
shutil.copytree(source / "templates/full", project)
shutil.copytree(
    source / "full_harness",
    project / "full_harness",
    ignore=shutil.ignore_patterns("__pycache__"),
)
(project / "README.md").write_text(
    "# CLI reading list\nA Python standard library command line program, no web UI.\n"
)
(project / "docs/00-global/project.md").write_text(
...
cfg = json.loads((project / ".harness/full.json").read_text())
cfg["checks"] = [
    {"name": "actual CLI round trip", "argv": [sys.executable, ".harness/check_cli.py"]}
]
cfg["agent_timeout"] = 900
write_json(project / ".harness/full.json", cfg)
session = root / "task"
session.mkdir()
task = {
    "number": 7,
    "title": "Build CLI reading list",
    "body": (project / "docs/00-global/project.md").read_text()
...
state = new_state(
    project, session, task, "owner/test", "local-fixture", "main", "probe-runner"
)
state["instruction"] = "按已确认需求交付，每阶段阅读可复用 Skills。"
state["run_id"] = "local-integration"
write_json(session / "state.json", state)
for stage in STAGES[:3]:
    print("starting", stage, flush=True)
    run_agent(project, session, state, stage)
    write_json(session / "state.json", state)
    print("stage", stage, state["status"], state.get("reason", ""), flush=True)
    if state["status"] == "awaiting_approval":
        begin(
            state,
            state["reply_token"] + " approve",
            "local-fixture",
            "probe-runner",
            "local-integration",
            session,
    ...
    if state["status"] != "running":
        break
if state["status"] == "running":
    print("starting independent review", flush=True)
    review_stage(project, session, state)
    write_json(session / "state.json", state)
print(
    "FINAL",
    json.dumps(
