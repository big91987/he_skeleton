...
cfg={'max_attempts':3,'agent_timeout':360,'check_timeout':20,'stages':{'development':{'artifact':'validation.md'}},'checks':[{'name':'required file contents','argv':[sys.executable,'.harness/check.py']}]}
context={'source':str(source),'workspace':str(work),'session':str(root),'evidence':str(evidence),'stage':'development','task':{'number':1},'config':cfg,'controls':controls(work),'deadline_monotonic':time.monotonic()+360}
