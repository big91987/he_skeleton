...
class AgentInputTests(unittest.TestCase):
    def setUp(self):
        ...
        self.state = {
            "task": {"number": 8, "body": "ORIGINAL_TASK_MARKER"},
            "instruction": "initial",
            "completed": {},
            "config": {
                "entries": ["AGENTS.md"],
                ...
                "stages": {
                    "requirements": {
                        "instruction": "define AC",
                        "skills": ["a"],
                        "artifact": "prd.md",
                    },
                    "design": {
                        "instruction": "design interfaces",
                        "skills": ["b"],
                        "artifact": "design.md",
                    },
                },
    ...
    def previous(self, packet, started=True):
        p = self.root / "turns/1/agent"
        p.mkdir(parents=True)
        write_json(p / "input.json", packet)
        if started:
            (p / "agent.jsonl").write_text(
                json.dumps({"type": "thread.started", "thread_id": "id"})
                + "\n"
    ...
    def test_resume_only_sends_new_answer(self):
        ...
        self.state["instruction"] = "search titles and body"
    ...
    def test_router_does_not_invoke_stage_skills(self):
        ...
        with patch.object(router, "invoke", return_value=(response, "id")) as call:
            router.classify(
                self.root, self.root, self.root, self.state, self.root / "evidence"
            )
...
class NativeCatalogTests(unittest.TestCase):
    def test_native_stage_allowlist_and_scope_switch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name, base in [
                ("stage-a", source / "full_harness/skills"),
                ("stage-b", source / "full_harness/skills"),
                ("stage-a", work / ".agents/skills"),
                ("unrelated", work / ".agents/skills"),
            ]:
                p = base / name
                p.mkdir(parents=True)
            ...
            for name in ["stage-a", "stage-b"]:
                ...
                self.assertEqual(
                    ...
                    (source / "full_harness/skills" / name / "SKILL.md").resolve(),
                )
                self.assertNotIn(body, (evidence / "skills.json").read_text())
            evidence = root / "no-skills"
            evidence.mkdir()
            self.assertEqual(configure(home, work, source, [], config, evidence), [])
            with self.assertRaises(ValueError):
                configure(home, work, source, ["../escape"], config, evidence)
