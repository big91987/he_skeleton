...
class ProjectPolicy(unittest.TestCase):
    def test_empty_and_python_project_need_no_html(self):
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            self.assertEqual(project.files(root), {})
    ...
    def test_browser_is_opt_in_and_invalid_verifier_is_not_substituted(self):
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            self.assertEqual(project.settings(root), {})
            (root / "harness-project.json").write_text(
                json.dumps({"verification": {"kind": "static-browser", "root": "ui"}})
            )
            self.assertEqual(project.settings(root)["verification"]["root"], "ui")
            (root / "harness-project.json").write_text(
                json.dumps({"verification": {"kind": "unknown"}})
            )
    ...
    def exercise(self, clarify):
        with tempfile.TemporaryDirectory() as d:
            ...
            event = {
                ...
                "sender": {"login": "owner"},
                "action": "opened",
                "issue": {"id": 77, "number": 7, "labels": [{"name": "harness"}]},
                "inputs": {"task": "7", "instruction": "Implement CLI"},
            }
            ep = root / "event.json"
            ep.write_text(json.dumps(event))
            env = {
                "GITHUB_EVENT_PATH": str(ep),
                "GITHUB_EVENT_NAME": "issues" if clarify else "workflow_dispatch",
            ...
            def agent(workspace, prompt, evidence, **kw):
                ...
                return {
                    "status": "ready",
                    "summary": "CLI implemented; verification pending",
                    "question": "",
                }
            ...
            def restore(repo, sha, target, **kw):
                ...
                if not clarify:
                    (target / "docs").mkdir()
                    (target / "docs/PRD.md").write_text("CLI product requirements")

            with (
                patch.dict(os.environ, env),
                patch.object(loop, "SOURCE", source),
                patch.object(loop, "gh", side_effect=api),
                patch.object(loop, "restore_app", side_effect=restore),
                patch.object(loop, "run_agent", side_effect=agent),
                patch.object(loop, "publish_code", return_value="new") as publish,
                patch.object(loop.subprocess, "run") as browser,
            ):
                loop.main()
                ...
                if clarify:
                    ...
                    self.assertFalse(
                        (root / "sessions/7/workspace/index.html").exists()
                    )

    def test_empty_project_clarification(self):
        self.exercise(True)
