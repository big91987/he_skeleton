    ...
    def test_install_upgrade_and_legacy_migration_on_both_paths(self):
        for transport in ('local', 'remote'):
            with self.subTest(transport=transport), tempfile.TemporaryDirectory() as folder:
                ...
                source.mkdir(); product.mkdir(); (product/'.git').mkdir()
                for name in [*MANAGED, *TEMPLATES.values()]:
                    p = source/name; p.parent.mkdir(parents=True, exist_ok=True); p.write_text('upstream-v1')
                def commit():
                ...
                def install():
                    ...
                    for name, data in changes.items():
                        p = product/name; p.parent.mkdir(parents=True, exist_ok=True); p.write_text(data)
                    return list(changes)
                commit()
                # An existing Owner workflow must survive even the first install.
                workflow, form = list(TEMPLATES)
                p = product/workflow; p.parent.mkdir(parents=True); p.write_text('owner-flow')
                installed = install()
                ...
                self.assertEqual(set(json.loads((product/'harness-upstream.json').read_text())['files']), set(MANAGED))
                # Simulate the old manifest that claimed both templates as managed.
                m = product/'harness-upstream.json'; legacy = json.loads(m.read_text())
                ...
                commit(); changed = install()
                self.assertFalse((product/workflow).exists())
                self.assertEqual((product/'.github/workflows/custom.yml').read_text(), 'owner-flow')
                ...
                (product/form).unlink()
                install()
                self.assertFalse((product/form).exists())  # intentional deletion stays deleted
                if transport == 'local':
                    before = {str(p.relative_to(product)): p.read_bytes() for p in product.rglob('*') if p.is_file()}
                    output = io.StringIO()
                    with contextlib.redirect_stdout(output):
                        self.assertEqual(sync(source, product, 'HEAD', True), [])
                    self.assertIn('upstream-template/', output.getvalue())
                    self.assertEqual(before, {str(p.relative_to(product)): p.read_bytes() for p in product.rglob('*') if p.is_file()})
