...
def main():
    ...
    p.add_argument(
        "repository", help="owner/product-repository; already created on GitHub"
    )
    ...
    p.add_argument(
        "--activate",
        action="store_true",
        help="publish managed files, configure Pages and start local Runner",
    )
    a = p.parse_args()
    ...
    if info["private"]:
        raise SystemExit(
            "This first static-preview bootstrap requires a public demo project for Pages"
        )
    checkout = a.directory.expanduser().resolve()
    source = Path(__file__).resolve().parents[1]
    if not checkout.exists():
        run(["gh", "repo", "clone", a.repository, str(checkout)])
    ...
    if origin.rstrip("/") not in {
        expected,
        expected[:-4],
        "git@github.com:" + a.repository + ".git",
    }:
    ...
    if info["default_branch"] != "main" or branch != "main":
        raise SystemExit(
            "Initial bootstrap currently requires the main branch checkout"
        )
    changed_files = sync(source, checkout, a.ref)
    if not a.activate:
        print(
            "Prepared files only. Review diff, then rerun on a clean checkout with --activate to deploy."
        )
        return
    ...
    if changed == 1:
        run(["git", "commit", "-m", "Update pinned Harness infrastructure"], checkout)
    elif changed != 0:
        raise SystemExit("Cannot inspect staged diff")
    ...
    if not any(x["name"] == "harness" for x in labels):
        api(
            "POST",
            "repos/" + a.repository + "/labels",
            {"name": "harness", "color": "2563EB"},
        )
    try:
        api("GET", "repos/" + a.repository + "/pages")
    ...
    for name in ["runtime", "sessions", "previews", "tools"]:
    ...
    if not (runtime / "config.sh").exists():
        ...
        asset = next(
            ...
            if x["name"].startswith("actions-runner-osx-arm64-")
        )
        digest = asset.get("digest", "")
        if not digest.startswith("sha256:"):
            raise SystemExit("Official Runner release lacks SHA256 digest")
    ...
    if registration.exists():
        ...
        if (
            saved.get("gitHubUrl", "").rstrip("/")
            != "https://github.com/" + a.repository
        ):
            raise SystemExit(
                "Runner belongs to another repository; refusing to reuse credentials"
            )
    else:
        token = api(
            "POST", "repos/" + a.repository + "/actions/runners/registration-token"
        )["token"]
        run(
            [
                "./config.sh",
    ...
    if not (runtime / ".service").exists():
        run(["./svc.sh", "install"], runtime)
    ...
    print(
        "Ready for an owner-created task; this is provisioning, not an end-to-end acceptance result."
    )
...
if __name__ == "__main__":
    try:
        ...
        status = (
            ...
            else "exit " + str(error.returncode)
        )
        raise SystemExit(
