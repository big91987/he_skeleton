...
def plan(destination, managed, templates, revision):
    ...
    def target(name):
        p = destination / name
        if not p.resolve().is_relative_to(destination):
        ...
        json.dumps(
            {
                ...
                "revision": revision,
                "files": {n: hashlib.sha256(b).hexdigest() for n, b in managed.items()},
            },
            indent=2,
        )
        + "\n"
    ...
    for name in changes:
        target(name)
    for name in set(old) - set(managed):
        changes[name] = None
    return {
        n: b
        for n, b in changes.items()
        if b is None or not target(n).exists() or target(n).read_bytes() != b
    }
...
def install(source, destination, ref):
    ...
    rev = subprocess.check_output(
        ["git", "rev-parse", "--verify", ref + "^{commit}"], cwd=source, text=True
    ).strip()
