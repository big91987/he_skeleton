...
TEMPLATES = {
    ".github/workflows/harness.yml": "templates/.github/workflows/harness.yml",
        ...
        print(
            "".join(
                difflib.unified_diff(
                    ...
                    contents[name].decode().splitlines(True),
                    fromfile="project/" + name,
                    tofile="upstream-template/" + name,
                )
            ),
            end="",
        )
...
def plan(destination, contents, revision):
    ...
    result["harness-upstream.json"] = (
        json.dumps(
            {
                "repository": "https://github.com/big91987/he_skeleton",
                "revision": revision,
...
def sync(source, destination, revision, show_template_diff=False):
    ...
    sha = subprocess.check_output(
        ["git", "rev-parse", "--verify", revision + "^{commit}"], cwd=source, text=True
    ).strip()
    paths = {name: name for name in MANAGED} | TEMPLATES
    contents = {
        name: subprocess.check_output(["git", "show", sha + ":" + path], cwd=source)
    ...
    print(
        "Synced Harness revision",
        sha,
        "; existing project workflows and forms preserved.",
    )
    return list(changes)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--template-diff",
        action="store_true",
        help="Only print template differences; write nothing",
    )
    a = p.parse_args()
