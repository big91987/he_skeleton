...
def command(event, event_name):
    ...
    if event_name == "workflow_dispatch":
        return (
            int(event["inputs"]["task"]),
            event["inputs"].get("instruction", "继续检查"),
            "dispatch-" + os.environ["GITHUB_RUN_ID"],
        )
    if event_name == "issues":
        issue = event["issue"]
        if event.get("action") != "opened" or not any(
            x["name"] == "harness" for x in issue["labels"]
        ):
...
def clarification_only(event_name, body, instruction):
    ...
    if explicit in {"clarify", "澄清"}:
        return True
    return event_name == "issues" and not re.search(
        r"### 开始方式\s+直接实施", body or ""
    )
...
def app_files(root):
    ...
    if (
        ...
        or len(files) > 100
        or sum(map(len, files.values())) > 10_000_000
    ):
...
def publish_code(repo, branch, base_sha, files, message, prefix_path="app/"):
    ...
    if head != base_sha:
        raise RuntimeError(
            "Branch changed during execution; refusing to overwrite external work"
        )
    ...
    old = {
        x["path"]: x
        for x in old_tree["tree"]
        if x["path"].startswith(prefix_path) and x["type"] == "blob"
    }
    tree = []
    for name, data in files.items():
        ...
        if old.get(path, {}).get("sha") != blob_sha:
            ...
            tree.append(
                {
                    "path": path,
                    "mode": old.get(path, {}).get("mode", "100644"),
                    "type": "blob",
                    "sha": blob["sha"],
                }
            )
...
def main():
    ...
    def delivery_metadata():
        return {
            ...
            "published_path": ("experiments/" + scope + "/" if scope else "main/")
            + state["preview"],
            "preview_kind": state.get("delivery", {}).get("preview_kind", "static-web"),
            "screenshots": [
                ["桌面运行截图", "screenshot.png"],
                ["手机运行截图", "mobile.png"],
            ],
        }
    ...
    def comment(body):
        ...
        gh(
            "POST",
            f"{prefix}/issues/{task}/comments",
            {
                "body": (
                    (
                        "实验分支：`"
                        + experiment
                        + "`。继续测试请在 Actions 选择同一分支运行，不使用主线 /harness 评论入口。\n\n"
                    )
                    if experiment
                    else ""
                )
    ...
    with (session / "lock").open("w") as lock:
        ...
        state = (
            ...
            else {"history": [], "processed": [], "round": 0}
        )
        ...
        if instruction.strip().lower() in {"stop", "停止"}:
            ...
            comment(
                "已停止后续推进，保留任务工作区和历史。正在执行的 Job 请在 Actions 点击 Cancel。"
            )
            return
        if instruction.strip().lower() in {"publish", "发布"}:
            if (
                not state.get("preview")
                or not (root / "previews" / state["preview"] / "index.html").is_file()
            ):
            ...
            atomic(
                output / "result.json",
                {
                    ...
                    "pr_url": state["pr_url"],
                    "sha": state.get("delivery", {}).get("sha", state["source_sha"]),
                    "event_id": event_id,
                    **delivery_metadata(),
                },
            )
        ...
        if "pull_request" in issue:
            ...
            branch = (
                f"codex/test-{scope}/task-{task}" if scope else f"codex/task-{task}"
            )
            try:
        ...
        if workspace.exists() and not state.get("workspace_version"):
            raise ValueError(
                "Legacy app-only session: start a new task to import the complete project"
            )
        ...
        elif "pull_request" in issue and state.get("source_sha") not in {
            None,
            source_sha,
        }:
        ...
        atomic(state_path, state)
        comment(
            f"开始第 {state['round']} 轮，恢复此任务已有工作区。完成后会回写结果；无需保持页面打开。"
        )
        evidence = session / f"round-{state['round']}"
        evidence.mkdir()
        try:
            prompt = project.prompt(issue, state["history"], config)
            read_only = clarification_only(
                os.environ["GITHUB_EVENT_NAME"], issue["body"], instruction
            )
            if read_only:
                prompt += (
            ...
            for attempt in range(3):
                ...
                test = subprocess.run(
                    [
                        "node",
                        str(SOURCE / "harness/browser.cjs"),
                        str(app),
                        str(evidence),
                        str(plan),
                    ],
            ...
            sha = publish_code(
                ...
                source_sha,
                files,
                f"Harness task #{task}, round {state['round']}",
                prefix_path="",
            )
            ...
            if not state.get("pr_url"):
                existing = gh(
                    "GET",
                    f"{prefix}/pulls?head="
                    + urllib.parse.quote(repo.split("/")[0] + ":" + branch)
                    + "&state=open",
                )
                state["pr_url"] = (
                    existing[0]["html_url"]
                    if existing
                    else f"https://github.com/{repo}/compare/{base}...{branch}?expand=1"
                )
            if not config.get("verification"):
                ...
                comment(
                    ...
                    + "\n\n代码/文档已保存：[查看改动]("
                    + state["pr_url"]
                    + ")。项目尚未配置可执行的验证器，本轮未通过自动验收；没有生成网页或发布应用预览。"
                )
                return
            files = app_files(app)
            ...
            (root / "previews/index.html").write_text(
                '<!doctype html><meta charset="utf-8"><title>Harness previews</title><h1>实验预览</h1><p>静态网页实验；每一轮链接固定，旧版本不会被新版本覆盖。</p><ul>'
                + "".join(
                    f'<li><a href="{p.relative_to(root / "previews").as_posix()}">{html.escape(str(p.parent.relative_to(root / "previews")))}</a></li>'
                    for p in links
                )
                + "</ul>"
            )
            ...
            atomic(state_path, state)
            comment(
                "本轮未完成，工作区和上下文已保留。错误类型：`"
                + type(error).__name__
                + "`。请查看执行日志；处理后回复 `/harness 重试`。"
            )
            raise
