...
def files(root):
    ...
    for path in root.rglob("*"):
        ...
        if (
            ...
            raise ValueError(
                "Potential credential file must not be published: "
...
def settings(source):
    ...
    if verification is not None:
        if (
            ...
            or verification.get("kind") != "static-browser"
        ):
            raise ValueError(
                "Requested verifier is not installed; do not substitute a web product"
            )
...
def check_control_changes(before, after):
    for name in before.keys() | after.keys():
        if name.split("/")[0] in PROTECTED and before.get(name) != after.get(name):
            raise ValueError(
                "Task cannot modify trusted Harness configuration: " + name
            )


def prompt(issue, history, config):
    text = (
        "Work on the user's project in the current workspace. Reply in Chinese. "
    ...
    if config.get("verification"):
        ...
        text += (
            "\nThis project explicitly enables static-browser verification at "
            + root
            + ". "
